...
| Q-4 | What is the best mechanism for cross-command behavioral standards? | Resolved | Inline mechanical standards (F1, F2, F5) at phase boundaries + `/gather` support skill for F3 gathering protocol (judgment standard — inline already failed per BUG-018, elevated to auto-invokable skill for reuse across all sessions) + per-skill F4 success messages. See DESIGN-005 DEC-2, SPEC-006. |
